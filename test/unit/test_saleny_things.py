...
def test_valid_salen_address():
    from salenlib import is_valid_salen_address

    main = valid_salen_address()
    test = valid_salen_address('testnet')

    assert is_valid_salen_address(main) is True
    assert is_valid_salen_address(main, 'mainnet') is True
    assert is_valid_salen_address(main, 'testnet') is False

    assert is_valid_salen_address(test) is False
    assert is_valid_salen_address(test, 'mainnet') is False
    assert is_valid_salen_address(test, 'testnet') is True


def test_invalid_salen_address():
    from salenlib import is_valid_salen_address

    main = invalid_salen_address()
    test = invalid_salen_address('testnet')

    assert is_valid_salen_address(main) is False
    assert is_valid_salen_address(main, 'mainnet') is False
    assert is_valid_salen_address(main, 'testnet') is False

    assert is_valid_salen_address(test) is False
    assert is_valid_salen_address(test, 'mainnet') is False
    assert is_valid_salen_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from salenlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % salenlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(salenlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(salenlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(salenlib.blocks_to_seconds(16616)) == 2612035
