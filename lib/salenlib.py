...
def is_valid_salen_address(address, network='mainnet'):
    ...
    salen_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Salen address
        return False

    if (address_version != salen_version):
        return False
...
# shims 'til we can fix the salend side
def SHIM_serialise_for_salend(sentinel_hex):
    ...
    # superblock => "trigger" in salend
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # salend expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    salend_hex = serialise(obj)
    return salend_hex


# shims 'til we can fix the salend side
def SHIM_deserialise_from_salend(salend_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(salend_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('salen.conf')
