...
def has_salen_conf():
    ...
    valid_salen_conf = False

    # ensure salen_conf exists & readable
    #
    # if not, print a message stating that Salen Core must be installed and
    # configured, including JSONRPC access in salen.conf
    try:
        f = io.open(config.salen_conf)
        valid_salen_conf = True
    except IOError as e:
        print(e)

    return valid_salen_conf
...
def main():
    ...
    if not has_salen_conf():
        print("SalenCore must be installed and configured, including JSONRPC access in salen.conf")
        sys.exit(1)
