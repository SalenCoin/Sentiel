...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, salend):
        golist = salend.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_salend(salend, item)
    ...
    @classmethod
    def import_gobject_from_salend(self, salend, rec):
        import decimal
        import salenlib
        import inflection
        ...
        # shim/salend conversion
        object_hex = salenlib.SHIM_deserialise_from_salend(object_hex)
        objects = salenlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from salend...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from salend, with every run
        ...
        # get/create, then sync payment amounts, etc. from salend - Salend is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(salend)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from salend! %s" % e)
            govobj.vote_delete(salend)
            return (govobj, None)
    ...
    def vote_delete(self, salend):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(salend, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, salend, signal, outcome):
        import salenlib
        ...
        output = salend.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = salenlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(salend, signal)

    def sync_network_vote(self, salend, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = salend.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 salen addr, non-multisig
            if not salenlib.is_valid_salen_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Salen address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (salenlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def get_prepare_command(self):
        import salenlib
        obj_data = salenlib.SHIM_serialise_for_salend(self.serialise())
    ...
    def prepare(self, salend):
        try:
            object_hash = salend.rpc_command(*self.get_prepare_command())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, salend):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - salend.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, salend):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - salend.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, salend):
        now = int(time.time())
        return (self.created_at < (now - salend.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, salend):
        if self.is_expired(salend):
            return False

        return True

    def is_deletable(self, salend):
        if self.is_expired(salend):
            return True
