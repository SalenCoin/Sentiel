...
class SalenDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some salend methods
    ...
    @classmethod
    def from_salen_conf(self, salen_dot_conf):
        from salen_config import SalenConfig
        config_text = SalenConfig.slurp_config_file(salen_dot_conf)
        creds = SalenConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = salenlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = salenlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = salenlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_salend_proto_version_with_sentinel_ping)
